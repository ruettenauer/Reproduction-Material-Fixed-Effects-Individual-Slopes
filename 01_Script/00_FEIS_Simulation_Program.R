...
fesim <- function(N = 300, time = 10, R = 30, seed = 123579, rob = FALSE, 
  ...
  ### Save parameters
  parameters <- c(N = N, time = time, R = R, seed = seed, rob = rob, 
  ...
  if(lagest == TRUE){
    beta_re.df <- matrix(NA, ncol = 4, nrow = R)
    beta_fe.df <- matrix(NA, ncol = 4, nrow = R)
    beta_feis.df <- matrix(NA, ncol = 4, nrow = R)
    colnames(beta_re.df) <- colnames(beta_fe.df) <-  colnames(beta_feis.df) <- c("x1", "lx1", "l2x1", "s")
    
    sd_re.df <- matrix(NA, ncol = 4, nrow = R)
    sd_fe.df <- matrix(NA, ncol = 4, nrow = R)
    sd_feis.df <- matrix(NA, ncol = 4, nrow = R)
  ...
  for(i in 1:R){
    ...
    ### Paste values
    beta_re.df[i, ] <- re.mod$coefficients[-1]
    beta_fe.df[i, ] <- fe.mod$coefficients
    beta_feis.df[i, 1:(ncol(beta_feis.df)-1)] <- feis.mod$coefficients
    
    sd_re.df[i, ] <- sqrt(diag(vcov(re.mod)))[-1]
    sd_fe.df[i, ] <- sqrt(diag(vcov(fe.mod)))
    sd_feis.df[i, 1:(ncol(sd_feis.df)-1)] <- sqrt(diag(feis.mod$vcov))
    
    test.df[i, 1] <- hf$wald_feis$result$chi2[1]
    test_p.df[i, 1] <- hf$wald_feis$result$chi2[3]
    
    test.df[i, 2] <- hf$wald_fe$result$chi2[1]
    test_p.df[i, 2] <- hf$wald_fe$result$chi2[3]
    
    test.df[i, 3] <- hf$wald_re$result$chi2[1]
    test_p.df[i, 3] <- hf$wald_re$result$chi2[3]
    
    creis[[i]] <- hf$CREIS
    
    theta_re.df[i, ] <- mean(re.mod$ercomp$theta)
    Va_re.df[i, ] <- re.mod$ercomp$sigma2[1]
    Ve_re.df[i, ] <- re.mod$ercomp$sigma2[2]
...
fesim_bs <- function(N = 300, time = 10, R = 30, seed = 123579, bsR = 10, tol = 1e-10, cov_emp = TRUE,
                     b1 = 0.3, bx1a1 = 1,  
                     cov_a2_bx1w = 0.85,
                     bx1w_m = 0.8, bx1w_sd = 1, 
                     a1_m = 0, a1_sd = 0, a2_m = 0, a2_sd = 1, 
  ...
  ### Save parameters
  parameters <- c(N = N, time = time, R = R, seed = seed, bsR = bsR, tol = tol, cov_emp = cov_emp,
                  b1 = b1, bx1a1 = bx1a1, 
                  cov_a2_bx1w = cov_a2_bx1w, 
                  bx1w_m = bx1w_m, bx1w_sd = bx1w_sd, 
                  a1_m = a1_m, a1_sd = a1_sd, a2_m = a2_m, a2_sd = a2_sd, 
  ...
  for(i in 1:R){
    ...
    ### Augmented Regression Test
    hf <- bsfeistest(feis.mod, type = "all", rep = bsR)
    
    
    ### Paste values
    beta_re.df[i, ] <- re.mod$coefficients[-1]
    beta_fe.df[i, ] <- fe.mod$coefficients
    beta_feis.df[i, 1] <- feis.mod$coefficients
    
    sd_re.df[i, ] <- sqrt(diag(vcov(re.mod)))[-1]
    sd_fe.df[i, ] <- sqrt(diag(vcov(fe.mod)))
    sd_feis.df[i, 1] <- sqrt(diag(feis.mod$vcov))
